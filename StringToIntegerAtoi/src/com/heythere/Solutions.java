...
public class Solutions {
    ...
    private String positive_negative_sign = "+-";//正负号
    ...
    public int myAtoi(String str) {
        ...
        if ('-' == str.charAt(executeIndex)) {
            return executeNegative(str, strLength, startIndex + 1);
        } else if ('+' == str.charAt(executeIndex)) {
        ...
        /*StringBuilder numberBuilder = new StringBuilder();//用StringBuilder构建字符串
        //处理正负号
        if ('+' == str.charAt(executeIndex)) {
        ...
        //处理符号后的数字
        while (executeIndex < strLength && digits.contains("" + (charToExecute = str.charAt(executeIndex)))) {
            numberBuilder.append(charToExecute);
            executeIndex++;
        }
        //若在正负号之后字符串中未添加数字字符
        if (1 == numberBuilder.length()) {
            return 0;
        }

        //当字符串中数字对int来说溢出时则返回Integer.MAX_VALUE或Integer.MIN_VALUE
        long numFromStr = Long.parseLong(numberBuilder.toString());
